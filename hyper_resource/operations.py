...
class Filter(CollectionOperation):
    ...
    def convert_to_expression(self, parameters_str):
        d = {
            ...
            "like": self.q_object_like
        }
        operator = parameters_str.split("/")[2]
        value = parameters_str.split("/")[4]
        if value in "and" or "or":
            second_operator = parameters_str.split('/')[6]
            return d[operator]("/".join(parameters_str.split("/")[:4])) & d[second_operator]("/".join(parameters_str.split("/")[4:]))
        else:
    ...
    def q_object_like(self, parameters_str):
        attribute_name = parameters_str.split("/")[1]
        operator = parameters_str.split("/")[2]
        value = parameters_str.split("/")[3]
        d = {attribute_name: value}
        ...
        try:
            ...
            if self.is_uri(param):
                ...
                try:
                    ...
                    #return GEOSGeometry(json.dumps(json.loads(response.text)))
